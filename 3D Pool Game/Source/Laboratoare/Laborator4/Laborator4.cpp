...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	//modelMatrix *= Transform3D::Translate(0, 0, 0); // e deja in centru; nu mai trebuie mutat
...
{
	float translateFactor = 5;
	float scaleFactor = 1;
	float rotatetionFactor = 1;

	if (window->KeyHold(GLFW_KEY_R)) {
		translateZ -= translateFactor * deltaTime;
	}
	else if (window->KeyHold(GLFW_KEY_F)) {
		translateZ += translateFactor * deltaTime;
	}
	else if (window->KeyHold(GLFW_KEY_A)) {
		translateX -= translateFactor * deltaTime;
	}
	else if (window->KeyHold(GLFW_KEY_D)) {
		translateX += translateFactor * deltaTime;
	}
	else if (window->KeyHold(GLFW_KEY_W)) {
		translateY += translateFactor * deltaTime;
	}
