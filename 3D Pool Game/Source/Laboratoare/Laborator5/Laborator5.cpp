...
void Laborator5::Init()
{
	...
	projectionMatrix = glm::perspective(RADIANS(perspectiveAngle), window->props.aspectRatio, 0.01f, 200.0f);
}
...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	// new objects
	{
		...
		modelMatrix = glm::rotate(modelMatrix, RADIANS(75.3f), glm::vec3(0, 1, 0));
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-8, 3, 0));
		modelMatrix = glm::rotate(modelMatrix, RADIANS(55.3f), glm::vec3(0, 0, 1));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_K)) {
		perspectiveAngle += deltaTime * step;
		projectionMatrix = glm::perspective(RADIANS(perspectiveAngle), window->props.aspectRatio, 0.01f, 200.0f);
	}
		...
		orthoHeight += deltaTime * step;
		projectionMatrix = glm::ortho(-10.0f, orthoWidth, -10.0f, orthoHeight, 0.01f, 200.0f);
	}
