...
void Laborator3::Update(float deltaTimeSeconds)
{
	...
	translateX += deltaTimeSeconds + 1;
	translateY += deltaTimeSeconds + 1;
	...
	if (degree > 180) {
		degree = 0;
		dist += 2 * step - squareSide;
	}

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(step + dist, 100 + (squareSide / 2));
	modelMatrix *= Transform2D::Rotate(-degree * M_PI / 180);
	modelMatrix *= Transform2D::Translate(-step, -(squareSide / 2));
	...
	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(translateX + 400, translateY + 250);
	modelMatrix *= Transform2D::Scale(scaleX + 0.4, scaleY + 0.25);
	//TODO create animations by multiplying current transform matrix with matrices from Transform 2D
	
	//RenderMesh2D(meshes["square2"], shaders["VertexColor"], modelMatrix);

	modelMatrix = glm::mat3(1);
	modelMatrix *= Transform2D::Translate(translateX + 650, translateY + 250);
	modelMatrix *= Transform2D::Scale(scaleX + 0.65, scaleY + 0.25);
